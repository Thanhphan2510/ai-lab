...
ABS_MIN_FACE = 20   # Minimum face size in pixels
ABS_MAX_FACE = 300  # Absolute maximum face size
CONF_THRESH_LOW = 0.25  # Base confidence threshold
MIN_FACE_AREA = 25*25  # Minimum face area
...
# -----------------------------------
# 2. Optimized Soft-Non Maximum Suppression
# -----------------------------------
def soft_nms(boxes, scores, iou_thr=0.3, sigma=0.5, thresh=0.001):
    """
    Optimized Soft-NMS implementation for face detection
    Args:
        boxes: List of bounding boxes [x, y, w, h]
        scores: Corresponding confidence scores
        iou_thr: IoU threshold for suppression
        sigma: Decay parameter for score adjustment
        thresh: Final score threshold
    Returns:
    ...
    # Process boxes in descending order of confidence
    for i in range(len(scores)):
        # Find max score in remaining boxes
        max_idx = i
...
def detect_faces(img, model, scaler, window_size):
    """
    Detect faces in an image using adaptive parameters
    Args:
        img: Input image (BGR format)
        model: Trained SVM classifier
        scaler: Feature scaler
        window_size: HOG window size (h, w)
    Returns:
        boxes: Detected face bounding boxes
        scores: Confidence scores
        decision_scores: All raw decision scores
    """
    ...
    # Determine image type (crowded or not)
    is_crowd_image = max_dim > 200 and min_dim > 200
    
    # Adaptive parameters based on image size and type
    if max_dim < 250:  # Small images
        ...
        nms_thr = 1.0
        sigma = 0.2
    elif is_crowd_image:  # Crowded images
        min_face_size = max(ABS_MIN_FACE, int(min_dim * 0.03))
        max_face_size = min(ABS_MAX_FACE, int(max_dim * 0.2))
        scales = [0.1, 0.2, 0.4, 0.6, 1.0, 1.2]
        step_size = 6
        nms_thr = 0.4
        sigma = 0.4
    else:  # Medium images (1-5 faces)
    ...
    print(f"Adaptive params: min_face={min_face_size}px, max_face={max_face_size}px, scales={len(scales)}, step={step_size}, sigma={sigma:.2f}, nms_thr={nms_thr:.2f}")
    ...
    for scale in scales:
        ...
        win_w = int(ww * scale)
        win_h = int(wh * scale)
        print(f"Processing scale {scale:.2f}: resized to {sw}x{sh}, window {win_w}x{win_h}, step {adaptive_step}")
        # Sliding window
        for y in range(0, sh - win_h + 1, adaptive_step):
            for x in range(0, sw - win_w + 1, adaptive_step):
                ...
                if score > CONF_THRESH_LOW:
                    ...
                    # Apply adaptive face size filtering
    ...
    if not filtered_boxes:
        return np.empty((0, 4)), np.array([]), decision_scores
    print(f"Detected {len(filtered_boxes)} before NMS")
    # Apply Soft-NMS
    boxes, scores = soft_nms(filtered_boxes, filtered_scores, sigma=sigma, thresh=nms_thr)
    print(f"Detected {len(boxes)} faces before NMS")
    # Cluster detections
    boxes, scores = cluster_detections(boxes, scores, original_shape, img, is_crowd_image)
    print(f"Detected {len(boxes)} faces after clustering and NMS")
...
def calculate_adaptive_threshold(positive_scores, all_scores):
    ...
    percentile_thresh = np.percentile(positive_scores, 50)
...
def cluster_detections(boxes, scores, img_shape, img, is_crowd_image=False):
    """Cluster detections using DBSCAN and merge results"""
    if len(boxes) == 0:
    ...
    if is_crowd_image:
        eps = min(img_shape[:2]) / 20
    else:
        eps = min(img_shape[:2]) / 12
...
def validate_face(img, box, score, skip_eye_check=False):
    """Validate detected face using multiple criteria"""
    x, y, w, h = map(int, box)
    ...
    for (ex, ey, ew, eh) in eyes:
        # Check eye position (should be in upper half of face)
        if ey < h * 0.6 and (ex > w*0.1 and ex+ew < w*0.9):
            valid_eyes += 1
    
    # Require at least 1 valid eye
    return valid_eyes >= 1
...
if __name__ == "__main__":
    ...
    test_images = []
    for i in range(1, 5):  # Test on multiple images
    ...
    for img_path in test_images:
        ...
        # Filter results using face validation
        valid_boxes = []
        valid_scores = []
        
        for box, score in zip(boxes, scores):
            print(f"   Validating box {box} with score {score:.2f}")
