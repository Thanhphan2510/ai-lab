...
# Optimized sliding window configuration
WINDOW_STEP = 8
...
def soft_nms(boxes, scores, iou_thr=0.2, sigma=0.6, thresh=0.001):
    """
    Improved Soft-NMS: Reduces confidence weights instead of complete removal
    """
    if len(boxes) == 0:
        return [], []
    ...
    for i in range(len(boxes)):
        ...
        # Reduce weights of overlapping boxes
...
def detect_faces(img, model, scaler, 
    ...
    for scale in scales:
        ...
        # Adjust scanning step based on scale
        ...
        for y in range(0, sh - int(wh*scale) + 1, adaptive_step):
            for x in range(0, sw - int(ww*scale) + 1, adaptive_step):
                ...
                # Ensure patch matches window_size
    ...
    # Soft-NMS + clustering remains unchanged
...
def cluster_detections(boxes, scores, img_shape):
    """
    Cluster nearby detections and compute average score
    """
    ...
    # Compute average results for each cluster
    final_boxes = []
    final_scores = []
    ...
    for label, detections in clusters.items():
        ...
        # Only keep clusters with high average score
...
def validate_face(img, box, min_valid_eyes=1):
    ...
    # For small faces: Use contrast + shape
    if w < 40 or h < 40:
    ...
    # Use eye cascade for sufficiently large faces
    ...
    # Accept if at least 1 eye is in upper half
...
if __name__ == "__main__":
    ...
    for img_path in test_images:
        ...
        # Filter results using face features
        valid_boxes = []
        valid_scores = []
        MIN_SCORE = 0.6
